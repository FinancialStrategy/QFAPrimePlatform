...
function plotTrackingDynamic(r){const pr=pointSeries(r.portfolio_daily_return_points||[],'Portfolio Daily Return');const br=pointSeries(r.benchmark_daily_return_points||[],'S&P 500 Daily Return');const n=Math.min(pr.length,br.length);let xs=[],cp=[],cb=[],diff=[];let p=1,b=1;for(let i=0;i<n;i++){xs.push(pr[i].x);p*=1+pr[i].y;b*=1+br[i].y;cp.push(p-1);cb.push(b-1);diff.push(pr[i].y-br[i].y);}const te=rollingStd(diff,63);plot('trackingDynamicPlot',[{type:'scatter',mode:'lines',name:`${r.meta.best_strategy} Cum Return`,x:xs,y:cp,line:{color:'#2E86AB',width:2},hovertemplate:'%{x}<br>Portfolio Cum: %{y:.2%}<extra></extra>'},{type:'scatter',mode:'lines',name:'Benchmark Cum Return',x:xs,y:cb,line:{color:'#E74C3C',width:1.5},hovertemplate:'%{x}<br>Benchmark Cum: %{y:.2%}<extra></extra>'},{type:'scatter',mode:'lines',name:'Rolling Tracking Error (63D)',x:xs,y:te,yaxis:'y2',line:{color:'#F39C12',width:1.5,dash:'dot'},hovertemplate:'%{x}<br>Rolling TE: %{y:.2%}<extra></extra>'}],baseChartLayout('Benchmark vs Tracking-Error Optimal Dynamic Curve',{xaxis:{title:'Date'},yaxis:{title:'Cumulative Return',tickformat:'.0%'},yaxis2:{title:'Rolling TE',overlaying:'y',side:'right',tickformat:'.0%',showgrid:false}}))}
...
const rvPts=r.rolling_volatility_daily_points||[];const arPts=r.active_return_daily_points||[];plot('rollingVolPlot',[dailyPctTrace(rvPts,'Rolling Annualized Volatility','Rolling Annualized Volatility',{line:{width:2,color:'#F39C12'}})],baseChartLayout(`Rolling Annualized Volatility — daily returns (${rvPts.length} observations)`,{xaxis:{title:'Date'},yaxis:{title:'Volatility',tickformat:'.0%'}}));plot('activeReturnPlot',[dailyPctTrace(arPts,'Cumulative Active Return','Cumulative Active Return',{line:{width:2,color:'#27AE60'}})],baseChartLayout(`Cumulative Active Return vs S&P 500 — DAILY points only (${arPts.length} observations)`,{xaxis:{title:'Date'},yaxis:{title:'Cumulative active return',tickformat:'.0%'}}));document.getElementById('stressKpiGrid').innerHTML=`<div class="kpi-card"><div class="kpi-label">Worst Scenario</div><div class="kpi-value">${r.stress_kpis.worst_scenario||'—'}</div><div class="kpi-sub">Impact: ${fmtPct(r.stress_kpis.worst_relative_return)}</div></div><div class="kpi-card"><div class="kpi-label">Avg Severity</div><div class="kpi-value">${fmtNum(r.stress_kpis.average_severity)}</div><div class="kpi-sub">Filtered scenarios</div></div><div class="kpi-card"><div class="kpi-label">Worst Drawdown Proxy</div><div class="kpi-value">${fmtPct(r.stress_kpis.worst_drawdown)}</div><div class="kpi-sub">Scenario loss estimate</div></div><div class="kpi-card"><div class="kpi-label">Scenario Count</div><div class="kpi-value">${r.stress_kpis.count}</div><div class="kpi-sub">Passing filters</div></div>`;plot('stressPlot',[{type:'bar',orientation:'h',y:r.stress_table.map(x=>x.Scenario).reverse(),x:r.stress_table.map(x=>x['Portfolio Impact']).reverse(),marker:{color:'#E74C3C'}}],baseChartLayout('Stress Scenario Impact Ranking',{xaxis:{tickformat:'.0%'}}))}
