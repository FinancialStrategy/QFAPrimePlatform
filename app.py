...
const catCounts={};metaRows.forEach(x=>{catCounts[x.category]=(catCounts[x.category]||0)+1});plot('infoHubPlot',[{type:'bar',x:Object.keys(catCounts),y:Object.values(catCounts),text:Object.values(catCounts),textposition:'outside'}],{title:'Investment Universe Identity Map'});plot('dashboardPlot',[{type:'bar',name:'Annual Return',x:r.strategy_metrics.map(x=>x.Strategy),y:r.strategy_metrics.map(x=>x['Annual Return']),text:r.strategy_metrics.map(x=>fmtPct(x['Annual Return'])),textposition:'outside'},{type:'bar',name:'Volatility',x:r.strategy_metrics.map(x=>x.Strategy),y:r.strategy_metrics.map(x=>x.Volatility),text:r.strategy_metrics.map(x=>fmtPct(x.Volatility)),textposition:'outside'},{type:'scatter',mode:'markers+lines',name:'Sharpe Ratio',x:r.strategy_metrics.map(x=>x.Strategy),y:r.strategy_metrics.map(x=>x['Sharpe Ratio']),yaxis:'y2',marker:{size:8}}],{title:'Executive Strategy Dashboard — Returns/Risk/Sharpe by Strategy',barmode:'group',xaxis:{tickangle:35},yaxis:{title:'Return / Volatility',tickformat:'.0%'},yaxis2:{title:'Sharpe Ratio',overlaying:'y',side:'right',showgrid:false}});plot('allocationPlot',[{type:'bar',x:r.weights.map(x=>x.asset),y:r.weights.map(x=>x.weight),text:r.weights.map(x=>fmtPct(x.weight)),textposition:'outside'}],{title:'Portfolio Allocation',yaxis:{tickformat:'.0%'}});const eqPts=r.equity_daily_points||[];const beqPts=r.benchmark_equity_daily_points||[];plot('equityPlot',[dailyMoneyTrace(eqPts,'Portfolio Equity Value','Portfolio Daily Equity',{line:{width:3}}),dailyMoneyTrace(beqPts,'S&P 500 Equity Value','S&P 500 Daily Equity',{line:{width:2,dash:'dot'},opacity:0.85})],dailyLayout(`Cumulative Equity Curve — DAILY points only (${eqPts.length} observations)`, 'Portfolio value (USD)', null));const ddPts=r.drawdown_daily_points||[];const bddPts=r.benchmark_drawdown_daily_points||[];plot('drawdownPlot',[dailyPctTrace(ddPts,'Portfolio Daily Drawdown','Portfolio DAILY Drawdown',{fill:'tozeroy',line:{width:1.6,shape:'linear',simplify:false}}),dailyPctTrace(bddPts,'S&P 500 Daily Drawdown','S&P 500 DAILY Drawdown',{line:{width:1.4,dash:'dot',shape:'linear',simplify:false},opacity:0.85})],dailyLayout(`Daily Drawdown — EVERY trading-day point (${ddPts.length} daily observations)`, 'Drawdown', '.0%'));plot('riskContribPlot',[{type:'bar',x:r.risk_contrib.map(x=>x.Asset),y:r.risk_contrib.map(x=>x['Contribution %']),text:r.risk_contrib.map(x=>fmtPct(x['Contribution %'])),textposition:'outside'}],{title:'Marginal Risk Contribution',yaxis:{tickformat:'.0%'}});plot('riskBarPlot',[{type:'bar',x:['VaR 95','CVaR 95','ES 95','Relative VaR 95','Relative CVaR 95'],y:[s.var_95,s.cvar_95,s.es_95,s.relative_var_95,s.relative_cvar_95]}],{title:'VaR / CVaR / ES Figures — Positive Loss Magnitudes',yaxis:{tickformat:'.0%'}});const rsPts=r.rolling_sharpe_daily_points||[];const rbPts=r.rolling_beta_daily_points||[];plot('rollingSharpePlot',[dailyTrace(rsPts,'Rolling Sharpe','Rolling Sharpe',{line:{width:2}})],dailyLayout(`Rolling Sharpe — daily returns, rolling window (${rsPts.length} observations)`, 'Sharpe Ratio', null));plot('rollingBetaPlot',[dailyTrace(rbPts,'Rolling Beta','Rolling Beta vs S&P 500',{line:{width:2}})],Object.assign(dailyLayout(`Rolling Beta — daily returns vs S&P 500 (${rbPts.length} observations)`, 'Beta', null),{shapes:[{type:'line',xref:'paper',x0:0,x1:1,y0:1,y1:1,line:{dash:'dash',color:'gray'}}]}));plot('pcaVariancePlot',[{type:'bar',x:r.pca_variance.map(x=>x.Component),y:r.pca_variance.map(x=>x['Explained Variance'])}],{title:'PCA Explained Variance',yaxis:{tickformat:'.0%'}});plot('pcaLoadingsPlot',[{type:'bar',x:r.pca_loadings.map(x=>x.Asset),y:r.pca_loadings.map(x=>x.PC1)}],{title:'PC1 Loadings'});
